...
class Game:
    ...
    def get_fewest_cubes(self) -> CubeSet:
        min_cubes = (0, 0, 0)
        for cube_set in self.cube_sets:
            min_cubes = (
                max(min_cubes[0], cube_set.red),
                max(min_cubes[1], cube_set.green),
                max(min_cubes[2], cube_set.blue),
            )
        return CubeSet(*min_cubes)

def solve(lines: list[str]) -> int:
    fewest_power_sum = 0
    for line in lines:
        game = Game(line)
        fewest_power_sum += game.get_fewest_cubes().power
    return fewest_power_sum
